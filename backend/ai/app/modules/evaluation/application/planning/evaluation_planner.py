...
class EvaluationPlanner:
    ...
    async def plan(self, request: PlanEvaluationRequest) -> EvaluationRunPlan:
        ...
        for metric_revision in benchmark.metric_revisions:
            ...
            if metric is None:
                raise EvaluationPlanningError(
                    f"MISSING_METRIC_DEFINITION:{metric_revision}"
                )
            if metric.revision != metric_revision:
                raise EvaluationPlanningError(
                    f"METRIC_IDENTITY_MISMATCH:{metric_revision}"
                )
        ...
        for grader_revision in benchmark.grader_revisions:
            ...
            if grader is None:
                raise EvaluationPlanningError(
                    f"MISSING_GRADER_DEFINITION:{grader_revision}"
                )
            if grader.revision != grader_revision:
                raise EvaluationPlanningError(
                    f"GRADER_IDENTITY_MISMATCH:{grader_revision}"
                )
            if not grader.calibration_required:
                continue

            calibration = await self._registry.get_calibration(grader_revision)
            if calibration is None:
                raise EvaluationPlanningError(
                    f"MISSING_GRADER_CALIBRATION:{grader_revision}"
                )
            if (
                ...
                raise EvaluationPlanningError(
                    f"MISMATCHED_GRADER_CALIBRATION:{grader_revision}"
                )
            if not calibration.calibrated_at <= now < calibration.expires_at:
                raise EvaluationPlanningError(
                    f"EXPIRED_GRADER_CALIBRATION:{grader_revision}"
                )
