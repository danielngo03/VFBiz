...
1. Resolve the active work item/release acceptance and named Release Owner. Pin
   route adapter, vehicle energy profile, charging station, connector, tariff,
   algorithm and cache-policy revisions used by the candidate.
...
3. Exercise routes needing zero, one and multiple stops, plus no feasible route. Verify totals equal the sum of travel, charging and cost segments within declared tolerances.
4. Test Google timeout, quota exhaustion, invalid response and unavailable mode. Confirm field masks, Autocomplete session tokens, deduplication, rate limits, cache policy and budget alerts.
5. Use record/replay for load tests. Limit real-provider smoke tests with an explicit budget cap.
6. Verify UI/API expose source revision, freshness, confidence range and an honest unavailable state instead of invented precision.
7. Capture latency, cache hit, provider request/cost and failure evidence.
...
8. Produce an immutable release report with candidate/base revision, observed
